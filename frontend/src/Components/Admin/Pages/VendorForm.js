  ...
  const submitHandler = async(values) =>{
    ...
    try {
      if (!/^[a-zA-Z\s]+$/.test(credentials.name)) {
        setMessage("Name must contain only alphabetic characters");
        // enqueueSnackbar("Name must contain only alphabetic characters", { variant: 'error' });
    ...
    if (response.ok) {
      ...
      console.log(error)
      message.error("Failed !! Vendor details inappropriate")      
    }
  }
  return (
    <>
